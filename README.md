...
Josh has obtained ProDeGe from IMG and run the SAGs through it.  We are unsure if the given taxonomy from the user input effects the results.  Testing this by changing the taxonomic level we specified and resubmitting.  Also going to send email to be sure we understand how ProDeGe is working.  Tentative plan to take what they gave us, if the taxonomic level doesn't matter, and resubmit to the family level if it does.
